...
pub async fn insert_or_update_tokens<'a, A: Acquire<'a, Database = Postgres>>(
    db: A,
    tokens: &[(i64, String, String, i64, Option<String>, String, bool)],
) -> sqlx::Result<()> {
    let mut conn = db.acquire().await?;

    let (chain_ids, denoms, display_symbols, decimals, logo_uris, display_names, gas_tokens): (
        Vec<i64>,
        Vec<String>,
        Vec<String>,
        Vec<i64>,
        ...
        .iter()
        .map(
            |(chain_id, denom, display_symbol, decimals, logo_uri, display_name, gas_token)| {
                (
                    *chain_id,
                    denom.clone(),
