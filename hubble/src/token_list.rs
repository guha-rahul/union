...
pub async fn update_tokens(db: sqlx::PgPool, urls: TokensUrls) -> Result<()> {
    if let Some(token_list) = get_tokens(urls).await? {
        ...
        if !token_inserts.is_empty() {
            ...
            let filtered_tokens: Vec<(i64, String, String, i64, Option<String>, String, bool)> =
                token_inserts
                    .iter()
                    .filter_map(|token| {
                        chain_ids_and_ids
                            .iter()
                            .find(|(chain_id, _)| *chain_id == token.0.to_string())
                            .map(|(_, id)| {
                                (
                                    *id as i64,
                                    token.1.clone(),
...
pub struct TokenInsert {
    ...
    pub gas_token: bool, // always false?
}
